...
bool MysqlMgr::AddFriendApply(const int& from, const int& to, const std::string& desc, const std::string& backname) {
	return _dao.AddFriendApply(from, to, desc, backname);
}
...
bool MysqlMgr::AddFriend(const int& from, const int& to, std::string back_name, std::vector<std::shared_ptr<AddFriendMsg>>& chat_datas) {
	return _dao.AddFriend(from, to, back_name, chat_datas);
}
