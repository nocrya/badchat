...
void LogicSystem::AddFriendApply(std::shared_ptr<CSession> session, const short& msg_id, const string& msg_data) {
	...
	auto backname = root["bakname"].asString();
	auto touid = root["touid"].asInt();
	std::cout << "user login uid is  " << uid << " applytext  is "
		<< desc << " bakname is " << backname << " touid is " << touid << endl;
	...
	//先更新数据库
	MysqlMgr::GetInstance()->AddFriendApply(uid, touid, desc, backname);
...
std::string LogicSystem::getCurrentTimeStamp() {
	...
	std::tm time_struct;
#if defined(_WIN32)
	localtime_s(&time_struct, &timestamp);  // Windows 安全版本
#else
	localtime_r(&timestamp, &time_struct);  // Linux/macOS 安全版本
#endif
...
void LogicSystem::DealChatImgMsg(std::shared_ptr<CSession> session, const short& msg_id, const string& msg_data)
{
	Json::Reader reader;
	Json::Value root;
	reader.parse(msg_data, root);
...
void LogicSystem::LoginHandler(shared_ptr<CSession> session, const short& msg_id, const string& msg_data) {
	...
	auto server_name = ConfigMgr::Inst().GetValue("SelfServer", "Name");
	{
		...
		RedisMgr::GetInstance()->Set(USERSESSIONPREFIX + uid_str, session->GetSessionId());
	}
	return;
}
...
bool LogicSystem::GetUserThreads(int userId, int lastId, int pageSize, std::vector<std::shared_ptr<ChatThreadInfo>>& threads, bool& loadMore, int& nextLastId)
{
	return MysqlMgr::GetInstance()->GetUserThreads(userId, lastId, pageSize, threads, loadMore, nextLastId);
}
