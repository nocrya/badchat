...
std::shared_ptr<FileInfo> RedisMgr::GetFileInfo(const std::string& md5)
{
	...
	// 2. 从 Redis 中获取 JSON 字符串
	std::string file_info_str;
	bool success = Get(redis_key, file_info_str); // 假设你的 RedisMgr 有 Get 方法

	// 3. 判断获取是否成功
	if (!success || file_info_str.empty()) {
		return nullptr; // 获取失败或数据为空
	}
	...
	Json::Value root;
	try {
		bool parsed = reader.parse(file_info_str, root);
		if (!parsed) {
			return nullptr; // JSON 解析失败
		}
	}
	catch (...) {
		return nullptr; // 捕获解析异常
	}
...
bool RedisMgr::SetDownLoadInfo(const std::string& md5, std::shared_ptr<FileInfo> file_info)
{
	Json::Reader reader;
	Json::Value root;
	root["file_path_str"] = file_info->_file_path_str;
	...
	root["trans_size"] = file_info->_trans_size;
	auto file_info_str = root.toStyledString();
	auto redis_key = "file_download_" + md5;
	bool success = SetExp(redis_key, file_info_str, 3600);
	return success;
}
