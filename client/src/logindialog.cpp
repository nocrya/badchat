    ...
    , ui(new Ui::LoginDialog)
{
    ...
    //连接tcp资源服务器
    connect(this, &LoginDialog::sig_connect_res_server, FileTcpMgr::GetInstance().get(), &FileTcpMgr::slot_tcp_connect);
    //连接tcp资源服务器成功信号
...
void LoginDialog::initHttpHandlers()
{
    ...
    _handlers.insert(ReqId::ID_LOGIN_USER, [this](QJsonObject jsonObj){
        ...
        qDebug()<< "email is " << email << " uid is " << _si->_uid << " chathost is "
                 << _si->_chat_host << " chatport is " << _si->_chat_port << " reshost is "
                 << _si->_res_host << " resport is " << _si->_res_port << " token is " << _si->_token;
...
{
    // 加载图片
    QPixmap originalPixmap(":/res/head_4.jpg");
        // 设置图片自动缩放
...
{
    if(bsuccess){
        showTip(tr("资源服务器连接成功，正在登录..."),true);
        QJsonObject jsonObj;
