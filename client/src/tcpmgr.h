...
private:
    ...
    bool _pending;
public slots:
    void slot_tcp_connect(std::shared_ptr<ServerInfo>);
...
signals:
    ...
    void sig_text_chat_msg(std::vector<std::shared_ptr<TextChatData>> chat_data);
    void sig_notify_offline();
    void sig_connection_closed();
    void sig_load_chat_thread(bool load_more, int next_last_id, std::vector<std::shared_ptr<ChatThreadInfo>> chat_threads);
    ...
    void sig_chat_img_rsp(int thread_id, std::shared_ptr<ImgChatData> msgdata);
};
