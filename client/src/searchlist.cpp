...
SearchList::SearchList(QWidget *parent):QListWidget(parent),_find_dlg(nullptr), _search_edit(nullptr), _send_pending(false), _add_user_item(nullptr)
{
    Q_UNUSED(parent);
...
void SearchList::UpdateSearchText(const QString& text)
{
    if (_add_user_item) {
        _add_user_item->SetSearchText(text);
    }
}
