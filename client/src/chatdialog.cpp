...
ChatDialog::ChatDialog(QWidget *parent)
    ...
    _b_loading(false), _cur_chat_thread_id(0), _last_widget(nullptr), _loading_dlg(nullptr), _cur_load_chat(nullptr)
{
    ui->setupUi(this);
    ...
    ui->horizontalLayout->removeWidget(ui->chat_user_wid);
    ui->horizontalLayout->removeWidget(ui->stackedWidget);
    splitter->addWidget(ui->chat_user_wid);
    ...
    ui->side_chat_wid->setProperty("state","normal");

    ui->side_chat_wid->SetState("normal","hover","pressed","selected_normal","selected_hover","selected_pressed");

    ui->side_contact_wid->SetState("normal","hover","pressed","selected_normal","selected_hover","selected_pressed");

    ui->side_settings_wid->SetState("normal","hover","pressed","selected_normal","selected_hover","selected_pressed");

    AddLBGroup(ui->side_chat_wid);
    AddLBGroup(ui->side_contact_wid);
    AddLBGroup(ui->side_settings_wid);

    connect(ui->side_chat_wid, &StateWidget::clicked, this, &ChatDialog::slot_side_chat);
    connect(ui->side_contact_wid, &StateWidget::clicked, this, &ChatDialog::slot_side_contact);
    connect(ui->side_settings_wid, &StateWidget::clicked, this, &ChatDialog::slot_side_settings);
    ...
    //设置聊天label选中状态
    ui->side_chat_wid->SetSelected(true);
    //设置选中条目
    SetSelectChatItem();
    //更新聊天界面信息
    SetSelectChatPage();
    ...
    _empty_page = new QWidget(this);
    _empty_page->setObjectName("empty_page");
    QVBoxLayout* emptyLayout = new QVBoxLayout(_empty_page);
    emptyLayout->addStretch();

    QLabel* logoLabel = new QLabel(_empty_page);
    logoLabel->setObjectName("empty_logo_lb");
    logoLabel->setFixedSize(120, 100);
    logoLabel->setAlignment(Qt::AlignCenter);
    QPixmap logoPix(120, 100);
    logoPix.fill(Qt::transparent);
    QPainter painter(&logoPix);
    painter.setRenderHint(QPainter::Antialiasing);
    painter.setPen(Qt::NoPen);
    painter.setBrush(QColor("#6BC5A0"));
    painter.drawEllipse(10, 10, 55, 55);
    painter.setBrush(QColor("#4AA87C"));
    painter.drawEllipse(50, 25, 55, 55);
    painter.end();
    logoLabel->setPixmap(logoPix);
    emptyLayout->addWidget(logoLabel, 0, Qt::AlignHCenter);

    QLabel* tipLabel = new QLabel(QStringLiteral("选择一个聊天开始交流"), _empty_page);
    tipLabel->setObjectName("empty_tip_lb");
    tipLabel->setAlignment(Qt::AlignCenter);
    emptyLayout->addWidget(tipLabel, 0, Qt::AlignHCenter);
    emptyLayout->addStretch();

    ui->stackedWidget->addWidget(_empty_page);
    ui->stackedWidget->setCurrentWidget(_empty_page);
    ...
    connect(_timer, &QTimer::timeout, this, [this](){
        ...
        //用于前端界面编辑，此时用户未登录，usermgr未初始化
        // 添加判空保护
        if (!user_info) {
            //qCritical() << "UserInfo is null! Check login status or UserMgr initialization.";
            return; // 提前退出
        }
        QJsonObject textObj;
...
void ChatDialog::showLoadingDlg(bool show, QString loading_text)
{
    if (show) {
        if (_loading_dlg) {
            _loading_dlg->deleteLater();
        }
        _loading_dlg = new LoadingDlg(this, loading_text);
...
void ChatDialog::UpdateChatMsg(std::vector<std::shared_ptr<TextChatData> > msgdata)
{
    for(auto & msg : msgdata){
        if(msg->GetSendUid() != _cur_chat_thread_id){
            break;
        }
...
void ChatDialog::SetSelectChatPage(int thread_id)
{
    ...
    if (thread_id == 0) {
        ...
        //设置信息
        auto user_info = con_item->GetChatData();
        ui->chat_page->SetChatData(user_info);
...
void ChatDialog::loadMoreChatUser()
{
    ...
    //         auto find_iter = _chat_thread_items.find(friend_ele->_uid);
    //         if(find_iter != _chat_thread_items.end()){
    //             continue;
    //         }
    ...
    //         _chat_thread_items.insert(friend_ele->_uid, item);
    //     }
...
void ChatDialog::slot_add_img_msg(int thread_id, std::shared_ptr<ImgChatData> msgdata) {
    auto chat_data = UserMgr::GetInstance()->GetChatThreadByThreadId(thread_id);
    if (chat_data == nullptr) {
        return;
    }

    chat_data->MoveMsg(msgdata);
    if (_cur_chat_thread_id != thread_id) {
        return;
    }

    //更新聊天界面信息
    ui->chat_page->UpdateChatStatus(msgdata->GetUniqueId(), msgdata->GetStatus());
...
void ChatDialog::slot_side_chat()
{
    ...
    ClearLabelState(ui->side_chat_wid);
...
void ChatDialog::slot_side_contact()
{
    ...
    ClearLabelState(ui->side_contact_wid);
...
{
    qDebug()<< "receive side settings clicked";
    ClearLabelState(ui->side_settings_wid);
...
void ChatDialog::slot_text_changed(const QString &str)
{
    ...
    {
        ShowSearch(true);
        ui->search_list->UpdateSearchText(str);
    }
}
...
void ChatDialog::slot_apply_friend(std::shared_ptr<AddFriendApply> apply)
{
    ...
    ui->side_contact_wid->ShowRedPoint(true);
...
void ChatDialog::slot_add_auth_friend(std::shared_ptr<AuthInfo> auth_info) {
    ...
    ui->chat_user_list->insertItem(0, item);
    ui->chat_user_list->setItemWidget(item, chat_user_wid);
    _chat_thread_items.insert(auth_info->_uid, item);
}
...
void ChatDialog::slot_auth_rsp(std::shared_ptr<AuthRsp> auth_rsp)
{
    ...
    ui->side_contact_wid->ShowRedPoint(false);
