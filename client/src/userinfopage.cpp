    ...
    ui(new Ui::UserInfoPage)
{
    ...
    //连接上
    //connect(ui->up_btn, &QPushButton::clicked, this, &UserInfoPage::on_up_btn_clicked);
...
void UserInfoPage::on_up_btn_clicked()
{
    ...
    _scaledPixmap = image.scaled( ui->head_lb->size(), Qt::KeepAspectRatio, Qt::SmoothTransformation); // 将图片缩放到label的大小
    ui->head_lb->setPixmap(_scaledPixmap); // 将缩放后的图片设置到QLabel上
    ui->head_lb->setScaledContents(true); // 设置QLabel自动缩放图片内容以适应大小
    qDebug()<<"hello";
}
...
QString UserInfoPage::generateUniqueIconName()
{
    ...
    // 生成一个随机数 (防止同一毫秒内多次调用重复)
    int random = QRandomGenerator::global()->bounded(100000);

    // 组合成文件名，使用 .png 作为后缀（头像通常用 png 或 jpg）
    return QString("%1_%2.png").arg(timestamp).arg(random);
}
...
void UserInfoPage::on_submit_btn_clicked()
{
    ...
    if (!_scaledPixmap.save(filePath, "PNG")) {
        QMessageBox::warning(
            this,
            tr("保存失败"),
    ...
    UserMgr::GetInstance()->AddMD5File(file_md5, fileInfo);
    //发送消息
