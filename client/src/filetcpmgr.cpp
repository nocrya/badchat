    ...
    _host(""), _port(0), _b_recv_pending(false), _message_id(0), _message_len(0), _bytes_sent(0), _pending(false)
{
    ...
    QObject::connect(&_socket, &QTcpSocket::connected, this, [&]() {
        qDebug() << "Connected to ResourceServer!";
...
void FileTcpMgr::slot_send_data(ReqId reqId, QByteArray data)
{
    uint16_t id = reqId;

    // 计算长度（使用网络字节序转换）
    quint32 len = static_cast<quint32>(data.length());
    ...
    qDebug() << "file tcp mgr send byte data is" << _current_block
...
void FileTcpMgr::slot_tcp_connect(std::shared_ptr<ServerInfo> si)
{
    ...
    // 尝试连接到资源服务器
    qDebug() << "Connecting to resserver...";
    _host = si->_res_host;
