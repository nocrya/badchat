...
TcpMgr::TcpMgr():_host(""),_port(0),_b_recv_pending(false),_message_id(0),_message_len(0), _pending(false)
{
    registerMetaType();
...
void TcpMgr::initHandlers()
{
    ...
    _handlers.insert(ID_SEARCH_USER_RSP, [this](ReqId id, int len, QByteArray data){
        ...
        if(!jsonObj.contains("error")){
            int err = ErrorCodes::ERR_JSON;
            qDebug() << "SearchUser Failed, err is Json Parse Err" << err ;
    ...
    _handlers.insert(ID_AUTH_FRIEND_RSP, [this](ReqId id, int len, QByteArray data) {
        ...
        for (const QJsonValue& data : jsonObj["chat_datas"].toArray()) {
            ...
            auto chat_time = data["chat_time"].toString();
    ...
    _handlers.insert(ID_NOTIFY_OFF_LINE_REQ, [this](ReqId id, int len, QByteArray data) {
        ...
        if (!jsonObj.contains("error")) {
            int err = ErrorCodes::ERR_JSON;
            qDebug() << "Notify Offline Failed, err is Json Parse Err" << err;
            return;
        }
